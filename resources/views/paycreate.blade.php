...
@section('content')
            ...
            <div class="col-6 card">
                ...
                <div class="card-body">
                    <form id="card-form" action="{{ route('payment.store') }}" method="POST">
                        ...
                        <input type="hidden" name="amount" value="{{$item_info->amount}}">
                        <input type="hidden" name="name" value="{{$item_info->name}}">
                        <input type="hidden" name="method" value={{$payment_method}}>
                        <button class="mt-3 btn btn-primary">支払い確定</button>
                    </form>
                </div>
            </div>
        </div>
    ...
    <script>
        /* 基本設定*/
        const stripe_public_key = "{{ config('cashier.key') }}"
